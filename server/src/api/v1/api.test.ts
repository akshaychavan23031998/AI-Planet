...
import vercelApp, { createApp, logger } from '../../app.js';
...
const expensePath = (
  ...
  `${claimPath(database)}/expenses/${database.data.expenses
    .find((item) => item.componentType === component)!
    ._id.toString()}`;
...
// All routes/middleware/services below are real.
// Only Mongo queries use isolated memory fixtures.

test('default Express export serves health for the Vercel runtime', async () => {
  assert.deepEqual((await request(vercelApp).get('/health').expect(200)).body, {
    status: 'ok',
  });
});
...
test('canonical submit returns 422 with findings and performs no write', async (t) => {
  ...
  assert.ok(
    ...
    ).error.details.findings.length > 0,
  );
...
for (const body of [
  {
    ...
    disallowedMinor: 0,
    reason: 'Incomplete sum',
  },
  {
    reimbursableMinor: 230400,
    disallowedMinor: 0,
  },
  {
    reimbursableMinor: -1,
    disallowedMinor: 230401,
    reason: 'Negative',
  },
  {
    reimbursableMinor: 0.5,
    disallowedMinor: 230399.5,
    reason: 'Fractional',
  },
  {
    reimbursableMinor: 230400,
...
test('business queue and read access follow the exact current RM then HOD', async (t) => {
  ...
  await post(`${path}/approve`, suresh, {
    remarks: 'Reviewed',
  }).expect(200);
