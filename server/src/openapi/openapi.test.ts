...
function expanded(
  ...
  if ('$ref' in value) {
    return schema(value.$ref.split('/').at(-1)!);
  }

  if (value.allOf?.length === 1) {
    return expanded(value.allOf[0]!);
  }
...
test('OpenAPI JSON is public and static with Mongo queries prohibited', async (t) => {
  ...
  assert.deepEqual(openapiDocument.servers, [
    {
      url: '/',
      description: 'Same backend origin',
    },
  ]);
});
...
test('production Swagger UI uses pinned CDN assets and the same OpenAPI document', async () => {
  const productionApp = createApp({
    NODE_ENV: 'production',
    CLIENT_ORIGIN: 'https://ai-planet-client.vercel.app',
  });

  const html = await request(productionApp)
    .get('/docs/')
    .expect(200)
    .expect('Content-Type', /html/);
  ...
  assert.match(
    html.text,
    /https:\/\/unpkg\.com\/swagger-ui-dist@5\.32\.15\/swagger-ui\.css/,
  );

  assert.match(
    html.text,
    /https:\/\/unpkg\.com\/swagger-ui-dist@5\.32\.15\/swagger-ui-bundle\.js/,
  );

  assert.match(
    html.text,
    /https:\/\/unpkg\.com\/swagger-ui-dist@5\.32\.15\/swagger-ui-standalone-preset\.js/,
  );

  assert.match(html.text, /url: '\/openapi\.json'/);

  assert.match(html.text, /persistAuthorization: true/);

  assert.equal(html.text.includes('src="/docs/swagger-ui-bundle.js"'), false);

  const specification = await request(productionApp)
    .get('/openapi.json')
    .expect(200)
    .expect('Content-Type', /json/);

  assert.deepEqual(specification.body, openapiDocument);
});
...
test('real HTTP errors and canonical settlement agree with documented envelopes/examples', async (t) => {
  ...
  assert.deepEqual(settlement.body, {
    data: canonicalSettlement,
  });
